...
func PlotLatLon(id int, subscriberName string, ch <-chan SubscribedMessage, results chan<- SubscriberResult) {
	...
	// Cartesian offsets may be negative. SmallestNonzeroFloat64 is positive,
	// so it is not a suitable initial maximum for coordinates below zero.
	...
	for msg := range ch {
		...
		// KS9E recordings expose GPS coordinates as top-level protobuf floats
		// instead of the older nested vn_gps message. Skip other schema layouts.
		decodedLat, latFound := data["vn_latitude"]
		decodedLon, lonFound := data["vn_longitude"]
...
func PlotTimeVelocity(id int, subscriberName string, ch <-chan SubscribedMessage, results chan<- SubscriberResult) {
	...
	for msg := range ch {
		...
		if veh_vec_floatDynamicMessage, found := data["current_rpms"].(*dynamic.Message); found {
			...
			// Validate both wheel fields before attempting their type assertions.
